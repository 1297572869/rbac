...
public class ConsoleInterceptor implements HandlerInterceptor, InitializingBean {
	...
	public boolean preHandle(HttpServletRequest request,
		...
		User user = findUserFromCookie(request);
        if (user == null) {
            String ticket = request.getParameter("ticket");
            user = findUserFromCache(ticket, request, response);
        }
        
        UserHolder.setCommonUser(user);
    ...
    private User findUserFromCookie(HttpServletRequest request)
            throws IOException {
		User user = null;
        if (user == null) {
            ...
            if (cookies != null) {
                for (Cookie cookie : cookies) {
                    if (cookie.getName().equals(UserHolder.COMMON_USER_SESSION_KEY)) {
    ...
    private User findUserFromCache(String key, HttpServletRequest request,
                                        HttpServletResponse response) throws IOException {
        if (key != null && !"".equals(key.trim())) {
            if (response != null) {
                Cookie cookie = new Cookie(UserHolder.COMMON_USER_SESSION_KEY, key);
                cookie.setPath("/");
