...
public class EntityScan {
    ...
    public void mybatisCustomizedInit(){
        Configuration c = sqlSessionTemplate.getConfiguration();
        Map<String, BaseMapper> map = SpringContext.getApplicationContext().getBeansOfType(BaseMapper.class);
            ...
            if(ArrayUtils.isNotEmpty(clazz)){
                ...
                if (ParameterizedType.class.isAssignableFrom(t[0].getClass())) {
                    for (Type t1:((ParameterizedType)t[0]).getActualTypeArguments()) {
                        ...
                        ReflectionUtils.setField(field, ms, rmList);
                        field.setAccessible(false);
                        for(ResultMapping rmg : rm.getResultMappings()){
                            if(StringUtils.isNotBlank(rmg.getNestedResultMapId())){
                                Field field11 = ReflectionUtils.findField(MappedStatement.class, "hasNestedResultMaps");
                                field11.setAccessible(true);
                                ReflectionUtils.setField(field11, ms, true);
                                field11.setAccessible(false);
                            }
                        }
                        ...
                        for(ResultMapping rmg : rm.getResultMappings()){
                            if(StringUtils.isNotBlank(rmg.getNestedResultMapId())){
                                Field field11 = ReflectionUtils.findField(MappedStatement.class, "hasNestedResultMaps");
                                field11.setAccessible(true);
                                ReflectionUtils.setField(field11, ms1, true);
                                field11.setAccessible(false);
                            }
                        }
                        ...
                        for(ResultMapping rmg : rm.getResultMappings()){
                            if(StringUtils.isNotBlank(rmg.getNestedResultMapId())){
                                Field field11 = ReflectionUtils.findField(MappedStatement.class, "hasNestedResultMaps");
                                field11.setAccessible(true);
                                ReflectionUtils.setField(field11, ms2, true);
                                field11.setAccessible(false);
                            }
                        }

                        MappedStatement ms5 = c.getMappedStatement(clazz[0].getName() + ".getInclude");
                        Field field5 = ReflectionUtils.findField(MappedStatement.class, "resultMaps");
                        field5.setAccessible(true);
                        ReflectionUtils.setField(field5, ms5, rmList);
                        field5.setAccessible(false);
                        for(ResultMapping rmg : rm.getResultMappings()){
                            if(StringUtils.isNotBlank(rmg.getNestedResultMapId())){
                                Field field11 = ReflectionUtils.findField(MappedStatement.class, "hasNestedResultMaps");
                                field11.setAccessible(true);
                                ReflectionUtils.setField(field11, ms5, true);
                                field11.setAccessible(false);
                            }
                        }

                        MappedStatement ms3 = c.getMappedStatement(clazz[0].getName() + ".insert");
                        if(ArrayUtils.isNotEmpty(ms3.getKeyProperties())) {
                            Field field3 = ReflectionUtils.findField(MappedStatement.class, "keyProperties");
                            ...
                            ReflectionUtils.setField(field3, ms3, keyProperties);
                            field3.setAccessible(false);
                        }
                        MappedStatement ms4 = c.getMappedStatement(clazz[0].getName() + ".insertExclude");
                        if(ArrayUtils.isNotEmpty(ms4.getKeyProperties())) {
                            Field field4 = ReflectionUtils.findField(MappedStatement.class, "keyProperties");
                            ...
                            ReflectionUtils.setField(field4, ms4, keyProperties);
    ...
    public static class ClassMapper{
        ...
        private Logger logger = LoggerFactory.getLogger(ClassMapper.class);
        ...
        public String selectSql(WhereCondition where, String[] includeProperty, String[] excludeProperty, String[] orderBy,
            ...
            for (FieldMapper fm : FieldMappers) {
                ...
                if(ArrayUtils.isNotEmpty(orderBy)){
                    for(int i=0; i<orderBy.length; i++){
                        if(orderBy[i]!=null&&orderBy[i].contains(fm.getProperty())) {
            ...
            if(max != null && first != null){
                sqlBuilder.append(" LIMIT ").append(first).append(", ").append(max);
            }else if(max != null){
                first = 0;
                sqlBuilder.append(" LIMIT ").append(first).append(", ").append(max);
            }
            if(where != null && where.isForUpdate()){
